...
err:
    ...
    xTaskNotify(mloop_task, MLOOP_EVT_TIME_UPDATE, eSetBits);
}
...
static void wifi_update(void)
{
    ...
    xTimerChangePeriod(outdoor_update_timer, pdMS_TO_TICKS(WIFI_UPDATE_INTERVAL), 0);
...
static void time_update(void)
{
    ...
    xTimerChangePeriod(outdoor_update_timer, pdMS_TO_TICKS(TIME_UPDATE_INTERVAL), 0);
...
static void inner_update(void)
{
    ...
    xTimerChangePeriod(outdoor_update_timer, pdMS_TO_TICKS(INNER_UPDATE_INTERVAL), 0);
...
{
    time_sync_timer = xTimerCreate("time sync", 1, pdFALSE, (void *)MLOOP_EVT_TIME_SYNC, mloop_timer_cb);
    ...
    time_update_timer = xTimerCreate("time update", pdMS_TO_TICKS(TIME_UPDATE_INTERVAL), pdTRUE, (void *)MLOOP_EVT_TIME_UPDATE, mloop_timer_cb);
    inner_update_timer = xTimerCreate("inner upadte", pdMS_TO_TICKS(INNER_UPDATE_INTERVAL), pdTRUE, (void *)MLOOP_EVT_INNER_UPDATE, mloop_timer_cb);
