...
uint64_t tim_now(void) //获取当前时间戳，单位为微秒
{
...
uint64_t tim_get_us(void)   //获取当前时间戳，单位为微秒
{
    return tim_now();
}

uint64_t tim_get_ms(void) //获取当前时间戳，单位为毫秒
{
    return tim_now() / 1000;
}

void tim_delay_us(uint32_t us)    //延时函数，参数us表示要延时的微秒数
{
    uint64_t now = tim_now();
    while (tim_now() - now < (uint64_t)us);
}

void tim_delay_ms(uint32_t ms)   //延时函数，参数ms表示要延时的毫秒数
{
    uint64_t now = tim_now();
